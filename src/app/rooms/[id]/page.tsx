...
// 🌟 Dynamically import the Map so Leaflet doesn't crash on Server-Side Rendering
const EditMap = dynamic(() => import('@/components/Map/EditMap'), { ssr: false });
...
const baseFacilityOptions = (t: any) => [
  { id: 'wifi', label: t('amenity.wifi.full') }, { id: 'ac', label: t('amenity.ac.full') },
  { id: 'kitchen', label: t('amenity.kitchen.full') }, { id: 'washer', label: t('amenity.washer.full') },
  { id: 'bathroom', label: t('amenity.bathroom.full') }, { id: 'workspace', label: t('amenity.workspace.full') }
];
...
export default function RoomDetailPage() {
  ...
  // (Leaflet map initialization moved to dynamic EditMap component to fix SSR)

  const handleGeocode = async (e: React.MouseEvent) => {
    e.preventDefault(); 
    if (!editForm.addressName.trim()) { alert(t('room.details.edit.addressEmpty')); return; }
    setIsGeocoding(true);
    try {
      ...
      if (data && data.length > 0) { 
        ...
        // Leaflet flyTo is now handled effectively inside the EditMap component's useEffect
      } else { alert(t('room.details.edit.geocodeNotFound')); }
    } catch (error) { alert(t('room.details.edit.networkError')); } finally { setIsGeocoding(false); }
  };

  const toggleLike = async () => {
    if (!currentUser) return alert(t('alert.loginRequired'));
  ...
  const handleChatWith = (targetUserId: string) => {
    if (!currentUser) {
      alert(t('room.details.chat.loginRequired'));
      return;
    }
  ...
  const handleRoomCountChange = (newCount: number) => {
    ...
    if (checkIn && checkOut) {
      ...
      if (selected.some(d => newBlocked.includes(d))) {
        ...
        alert(t('room.details.apply.stockError', { count: newCount }));
      }
    }
  };

  const handleApply = async () => {
    if (!currentUser) { alert(t('alert.loginRequired')); return; }
    if (!checkIn || !checkOut) { alert(t('alert.selectDates')); return; }
    ...
    try {
      ...
      setMyBooking(data); 
      alert(t('room.details.apply.success')); window.location.reload(); 
    } catch (err: any) { alert(t('room.details.apply.failed', { error: err.message })); } finally { setIsApplying(false); }
  };
    ...
    try {
      const res = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({
          ...
          price: room.price,
          hostId: room.author_id,
          customHostUrl: window.location.origin,
        }),
      });
      const data = await res.json();
      if (data.url) window.location.href = data.url;
      else alert(t('alert.payFailed') + ': ' + (data.error || 'unknown'));
    } catch (err: any) { alert(t('alert.payError') + ': ' + err.message); }
  };
  ...
  const handleAddCustomAmenity = () => {
    ...
    if (!customAmenities.find(a => a.id === newId) && !baseFacilityOptions(t).find(a => a.id === newId)) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setCurrentImgIndex(0); setIsEditModalOpen(false); alert(t('room.details.edit.success'));
    } catch (err: any) { alert(t('room.details.edit.failed', { error: err.message })); } finally { setIsUpdating(false); }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      // 3. 确认干干净净，二次弹窗让用户确认
      if (!window.confirm(t('room.details.delete.confirmTitle') + "\n" + t('room.details.delete.confirmMsg'))) return;
      ...
      if (!data || data.length === 0) {
         throw new Error(t('room.details.delete.dbError'));
      }

      alert(t('room.details.delete.success'));
      router.push('/my-rooms');
      
    } catch (err: any) {
      alert(t('room.details.delete.failed', { error: err.message }));
    }
  };
  ...
  if (!room) return <div className="min-h-screen flex items-center justify-center text-gray-500">{t('rooms.empty')}</div>;
        ...
        <div className="p-5 bg-white rounded-t-[24px] -mt-6 relative z-10 min-h-[50vh]">
          ...
          <div onClick={() => router.push(`/user/${room.author_id}`)} className="flex items-center justify-between mb-6 pb-6 border-b border-gray-100 cursor-pointer hover:bg-gray-50 p-2 -ml-2 rounded-xl transition-colors group">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[14px] font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{t('room.details.host')}: {room.author_name}</div>
                <div className="flex items-center gap-1 text-gray-400 mt-0.5">
                  <svg fill="currentColor" viewBox="0 0 24 24" className="w-3 h-3 text-green-500"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
                  <span className="text-[11px] font-medium">{t('room.details.verified')}</span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gray-50 rounded-2xl p-4 mb-6 grid grid-cols-2 gap-y-4 gap-x-2 border border-gray-100/50">
            <div className="flex flex-col gap-1.5"><span className="text-[12px] text-gray-400 font-medium flex items-center gap-1.5"><svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" /></svg>{t('room.details.rentMode')}</span>
              <span className="text-[14px] font-bold text-gray-800">{room.rent_mode === 'entire' ? t('room.details.rentMode.entire') : t('room.details.rentMode.room') + ' ' + t('room.details.rentMode.total', { count: room.total_rooms || 1 })}</span>
            </div>
            <div className="flex flex-col gap-1.5"><span className="text-[12px] text-gray-400 font-medium flex items-center gap-1.5"><svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{t('room.details.price')}</span>
               <span className="text-[14px] font-bold text-blue-600">{room.price_amount ? `${room.price_amount} ${room.price_currency} ${t('room.details.price.night')}` : t('room.details.price.negotiate')}</span>
            </div>
            <div className="flex flex-col gap-1.5"><span className="text-[12px] text-gray-400 font-medium flex items-center gap-1.5"><svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 21v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21m0 0h4.5V3.75M5.25 9h3.75m-3.75 3h3.75m-3.75 3h3.75m-3.75-6h3.75m-3.75 3h3.75m-3.75 3h3.75M9 21v-8.25" /></svg>{t('room.details.type')}</span>
              <span className="text-[14px] font-bold text-gray-800">
                {lang === 'zh' ? room.room_type : (
                  room.room_type === '独立单间' ? t('amenity.privateRoom') :
                  room.room_type === '整套出租' ? t('room.details.rentMode.entire') :
                  room.room_type === '找室友' ? 'Find Roommate' :
                  room.room_type === '沙发客' ? 'Couchsurfing' : room.room_type
                )}
              </span>
            </div>
            <div className="flex flex-col gap-1.5"><span className="text-[12px] text-gray-400 font-medium flex items-center gap-1.5"><svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" /></svg>{t('room.details.availableDate')}</span><span className="text-[14px] font-bold text-gray-800">{room.available_date ? t('room.details.availableFrom', { date: room.available_date }) : t('room.details.availableNow')}</span></div>
          </div>

          {(room.address_name || room.city_name) && (
            <div className="mb-8">
              <h3 className="text-[16px] font-bold text-gray-900 mb-3 flex items-center gap-2"><span className="w-1 h-4 bg-blue-500 rounded-full"></span>{t('room.details.location')}</h3>
              <div className="border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                <div className="p-4 bg-white flex items-center justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="text-[15px] font-bold text-gray-900 truncate">{room.address_name || `${room.city_name} (${t('room.details.location.none')})`}</div>
                  </div>
                ...
                <div className="h-44 bg-gray-100 relative w-full overflow-hidden border-t border-gray-100">
                  {room.latitude && room.longitude ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/5 backdrop-blur-sm"><span className="text-gray-500 text-[13px] font-bold px-4 py-2 bg-white/80 rounded-full shadow-sm">{t('room.details.location.noCoord')}</span></div>
                  )}
                </div>
              </div>
            </div>
          ...
          {validAmenities.length > 0 && (
            <div className="mb-8">
              <h3 className="text-[16px] font-bold text-gray-900 mb-4 flex items-center gap-2"><span className="w-1 h-4 bg-blue-500 rounded-full"></span>{t('room.details.amenities')}</h3>
              <div className="grid grid-cols-3 gap-y-4 gap-x-2">
                {validAmenities.map((item: string) => {
                  ...
                  const label = item === 'wifi' ? t('amenity.wifi.full') : item === 'ac' ? t('amenity.ac.full') : item === 'kitchen' ? t('amenity.kitchen.full') : item === 'washer' ? t('amenity.washer.full') : item === 'bathroom' ? t('amenity.bathroom.full') : item === 'workspace' ? t('amenity.workspace.full') : item;
                  return (
          ...
          <div className="mb-8">
            <h3 className="text-[16px] font-bold text-gray-900 mb-3 flex items-center gap-2"><span className="w-1 h-4 bg-blue-500 rounded-full"></span>{t('room.details.intro')}</h3>
          ...
          {isHost && (
            <div className="mb-4 p-5 bg-blue-50/40 rounded-2xl border border-blue-100">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[15px] font-bold text-gray-900 flex items-center gap-2"><span className="w-1 h-4 bg-blue-500 rounded-full"></span>{t('room.details.statusBoard')} <span className="text-[11px] font-normal text-blue-500 bg-blue-100/50 px-2 py-0.5 rounded-full">{t('room.details.hostOnly')}</span></h3>
                <div className="flex items-center gap-2 bg-white px-2 py-1 rounded-lg border border-gray-200 shadow-sm">
                  ...
                  <div className="text-[12px] font-bold text-gray-800 min-w-[65px] text-center">{t('room.details.calendar.yearMonth', { year: calendarViewDate.getFullYear(), month: calendarViewDate.getMonth() + 1 })}</div>
                  <button onClick={() => setCalendarViewDate(new Date(calendarViewDate.getFullYear(), calendarViewDate.getMonth() + 1, 1))} className="w-6 h-6 flex items-center justify-center bg-gray-50 rounded-full hover:bg-gray-200 text-gray-600 transition-colors text-xs">&gt;</button>
                </div>
              </div>
              
              <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                <div className="grid grid-cols-7 gap-1 text-center mb-2">{(lang === 'zh' ? ['日', '一', '二', '三', '四', '五', '六'] : ['S', 'M', 'T', 'W', 'T', 'F', 'S']).map(d => <div key={d} className="text-[11px] font-bold text-gray-400">{d}</div>)}</div>
                ...
                <div className="flex flex-wrap gap-4 text-xs font-medium text-gray-600 justify-center mt-5 bg-gray-50 p-2.5 rounded-xl border border-gray-100">
                  <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-red-50 border border-red-200 block"></span>{t('room.details.calendar.full')}</div>
                  {(room.total_rooms || 1) > 1 && (<div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-orange-50 border border-orange-200 block"></span>{t('room.details.calendar.partial')}</div>)}
                  <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-white border border-gray-200 block"></span>{t('room.details.calendar.free')}</div>
                </div>
              </div>
            </div>
          )}
                ...
                <div className="space-y-3">
                  ...
                  {room.rent_mode !== 'entire' && (room.total_rooms || 1) > 1 && (
                    <div className="flex items-center justify-between bg-white border border-gray-200 rounded-xl p-3">
                      <span className="text-[13px] font-bold text-gray-500">{t('room.details.apply.roomCount')} <span className="text-[10px] font-normal text-gray-400 ml-1">{t('room.details.apply.totalCount', { count: room.total_rooms })}</span></span>
                      <div className="flex items-center gap-3">
                        ...
                        <button type="button" onClick={() => handleRoomCountChange(Math.min(room.total_rooms || 1, roomCount + 1))} className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-lg transition-colors ${roomCount >= room.total_rooms ? 'bg-gray-50 text-gray-300' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`} disabled={roomCount >= (room.total_rooms || 1)}>+</button>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-1">
                    <div onClick={() => { setSelectingType('checkIn'); setCalendarViewDate(checkIn ? new Date(checkIn) : new Date()); }} className={`border rounded-xl p-3 relative cursor-pointer transition-all ${selectingType === 'checkIn' ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50/30' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1">{t('room.details.apply.checkIn')}</span>
                      <div className="text-[15px] font-bold text-gray-900">{checkIn || t('room.details.apply.selectDate')}</div>
                    </div>
                    <div onClick={() => { if (!checkIn) { setSelectingType('checkIn'); setCalendarViewDate(new Date()); } else { setSelectingType('checkOut'); setCalendarViewDate(checkOut ? new Date(checkOut) : new Date(checkIn)); } }} className={`border rounded-xl p-3 relative cursor-pointer transition-all ${selectingType === 'checkOut' ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50/30' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1">{t('room.details.apply.checkOut')}</span>
                      <div className="text-[15px] font-bold text-gray-900">{checkOut || t('room.details.apply.selectDate')}</div>
                    </div>
                  </div>
                  ...
                  {selectingType && (
                    <div className="bg-white border border-gray-200 shadow-2xl rounded-2xl p-4 relative animate-fade-in-up">
                      ...
                      <div className="text-[14px] font-black text-gray-900 mb-4 pl-2">{selectingType === 'checkIn' ? t('room.details.apply.step1') : t('room.details.apply.step2')}</div>
                      <div className="flex justify-between items-center mb-4 px-2">
                        ...
                        <div className="text-[15px] font-bold text-gray-800">{t('room.details.calendar.yearMonth', { year: calendarViewDate.getFullYear(), month: calendarViewDate.getMonth() + 1 })}</div>
                        <button onClick={() => setCalendarViewDate(new Date(calendarViewDate.getFullYear(), calendarViewDate.getMonth() + 1, 1))} className="w-8 h-8 flex items-center justify-center bg-gray-50 rounded-full hover:bg-gray-200 text-gray-600 transition-colors">&gt;</button>
                      </div>
                      <div className="grid grid-cols-7 gap-1 text-center mb-2">{(lang === 'zh' ? ['日', '一', '二', '三', '四', '五', '六'] : ['S', 'M', 'T', 'W', 'T', 'F', 'S']).map(d => <div key={d} className="text-[11px] font-bold text-gray-400">{d}</div>)}</div>
                  ...
                  <textarea value={guestMessage} onChange={(e) => setGuestMessage(e.target.value)} placeholder={t('room.details.apply.message')} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm outline-none focus:border-blue-500 resize-none mt-2" rows={2}></textarea>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in-up">
          <div className="bg-white rounded-[24px] w-full max-w-xl max-h-[90vh] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-gray-100 shrink-0">
              <h2 className="text-xl font-black text-gray-900">{t('room.details.edit.title')}</h2>
            ...
            <div className="p-6 overflow-y-auto space-y-5">
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                <label className="block text-xs font-bold text-gray-700 mb-3">{t('room.details.edit.images')}</label>
                <div className="flex gap-3 mb-4">
                   <button onClick={() => setEditForm({...editForm, imageMode: 'system'})} className={`flex-1 py-2 rounded-lg text-[13px] font-bold border transition-all ${editForm.imageMode === 'system' ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-white text-gray-500 border-gray-200'}`}>{t('room.details.edit.imageSystem')}</button>
                   <button onClick={() => setEditForm({...editForm, imageMode: 'custom'})} className={`flex-1 py-2 rounded-lg text-[13px] font-bold border transition-all ${editForm.imageMode === 'custom' ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-white text-gray-500 border-gray-200'}`}>{t('room.details.edit.imageCustom')}</button>
                </div>
                {editForm.imageMode === 'system' ? (
                  <div className="text-[12px] text-gray-400 bg-white p-3 rounded-lg border border-dashed border-gray-200 text-center">{t('room.details.edit.imageHint')}</div>
                ) : (
                  <div>
              ...
              <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100 shadow-sm">
                <label className="block text-[13px] font-bold text-gray-900 mb-2 flex items-center gap-1.5">
                  ...
                  {t('room.details.location')}
                </label>
                
                <div className="flex gap-2 mb-3">
                  <input 
                    type="text" 
                    placeholder={t('room.details.edit.addressHint')} 
                  ...
                  <button onClick={handleGeocode} disabled={isGeocoding} className="px-4 py-2 bg-gray-900 text-white text-xs font-bold rounded-lg hover:bg-black whitespace-nowrap disabled:opacity-50 transition-colors flex items-center gap-1">
                    {isGeocoding ? <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : (lang === 'zh' ? '搜索并锁定' : 'Search')}
                  </button>
                </div>
                ...
                <div className="relative w-full h-40 rounded-xl overflow-hidden border border-gray-200 shadow-inner group">
                  {editForm.lat && editForm.lng ? (
                    <EditMap 
                      ...
                      isMapContainerReady={isEditModalOpen}
                    />
                  ) : (
                    <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 gap-2 bg-gray-50/50">
                      <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 opacity-50"><path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" /></svg>
                      <span className="text-sm font-medium">{t('room.details.location.none')}</span>
                    </div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-xs font-bold text-gray-700 mb-1">{t('publish.city')}</label><input type="text" value={editForm.city} onChange={(e) => setEditForm({...editForm, city: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500/20" /></div>
                <div><label className="block text-xs font-bold text-gray-700 mb-1">{t('room.details.availableDate')}</label><input type="date" value={editForm.availableDate} onChange={(e) => setEditForm({...editForm, availableDate: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500/20" /></div>
              </div>
              <div><label className="block text-xs font-bold text-gray-700 mb-1">{t('publish.title')}</label><input type="text" value={editForm.title} onChange={(e) => setEditForm({...editForm, title: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500/20" /></div>

              <div className="grid grid-cols-2 gap-4 bg-gray-50/50 p-3 rounded-xl border border-gray-100/50">
                <div><label className="block text-xs font-bold text-gray-700 mb-1">{t('room.details.rentMode')}</label><select value={editForm.rentMode} onChange={(e) => setEditForm({...editForm, rentMode: e.target.value})} className="w-full bg-white border border-gray-200 rounded-lg px-2 py-2 text-xs font-medium focus:ring-2 focus:ring-blue-500/20 outline-none"><option value="entire">{t('room.details.rentMode.entire')}</option><option value="room">{t('room.details.rentMode.room')}</option></select></div>
                <div><label className="block text-xs font-bold text-gray-700 mb-1">{t('publish.totalRooms')}</label><input type="number" min="1" disabled={editForm.rentMode === 'entire'} value={editForm.totalRooms} onChange={(e) => setEditForm({...editForm, totalRooms: Number(e.target.value)})} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-blue-500/20 disabled:bg-gray-100 disabled:text-gray-400" /></div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1">{t('room.details.price')}</label>
                ...
                <div><label className="block text-xs font-bold text-gray-700 mb-1">{t('room.details.type')}</label><select value={editForm.roomType} onChange={(e) => setEditForm({...editForm, roomType: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-2 py-2 text-xs font-medium outline-none focus:ring-2 focus:ring-blue-500/20"><option>{t('amenity.privateRoom')}</option><option>{t('room.details.rentMode.entire')}</option><option>{lang === 'zh' ? '找室友' : 'Find Roommate'}</option><option>{lang === 'zh' ? '沙发客' : 'Couchsurfing'}</option></select></div>
              </div>

              <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                <label className="block text-xs font-bold text-gray-700 mb-2">{t('room.details.amenities')} ({lang === 'zh' ? '多选' : 'Multiple Select'})</label>
                <div className="flex flex-wrap gap-2 mb-3">
                  {[...baseFacilityOptions(t), ...customAmenities].map(option => (
                ...
                <div className="flex gap-2">
                   <input type="text" placeholder={lang === 'zh' ? '例如：免费停车、宠物友好...' : 'e.g. Free Parking, Pet Friendly...'} value={newAmenity} onChange={(e) => setNewAmenity(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), handleAddCustomAmenity())} className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 text-xs outline-none focus:ring-1 focus:ring-blue-500" />
                   <button type="button" onClick={handleAddCustomAmenity} className="px-3 py-1.5 bg-gray-900 text-white text-xs font-bold rounded-lg hover:bg-black">{lang === 'zh' ? '添加' : 'Add'}</button>
                </div>
              </div>

              <div><label className="block text-xs font-bold text-gray-700 mb-1">{t('publish.description')}</label><textarea rows={3} value={editForm.description} onChange={(e) => setEditForm({...editForm, description: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none resize-none focus:ring-2 focus:ring-blue-500/20"></textarea></div>
            </div>

            <div className="p-4 border-t border-gray-100 flex justify-end gap-3 shrink-0">
              <button onClick={() => setIsEditModalOpen(false)} className="px-5 py-2 text-sm font-bold text-gray-500 bg-gray-100 rounded-full hover:bg-gray-200 transition">{t('publish.cancel') || (lang === 'zh' ? '取消' : 'Cancel')}</button>
              <button onClick={handleUpdate} disabled={isUpdating} className="px-6 py-2 text-sm font-bold text-white bg-blue-600 rounded-full hover:bg-blue-700 transition shadow-sm">
                {isUpdating ? t('room.details.edit.saving') : t('room.details.edit.save')}
              </button>
            </div>
          </div>
        </div>
