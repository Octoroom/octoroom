...
export default function RoomDetailPage() {
  ...
  // 🌟 核心杀 Bug：只有完全匹配 UUID，才算是真实房东！防止 undefined === undefined 导致游客拥有房东权限
  const isHost = Boolean(currentUser?.id && room?.author_id && currentUser.id === room.author_id);
  ...
  const displayImages = room.cover_image && room.cover_image.includes('http') ? room.cover_image.split(',').map((s:string) => s.trim()).filter(Boolean) : [roomId ? `https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=800&q=80&random=${roomId}` : ''];
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide pb-[320px]">
        <div className="w-full h-[300px] relative group bg-gray-100">
          <div 
             ...
             onScroll={(e) => setCurrentImgIndex(Math.round(e.currentTarget.scrollLeft / e.currentTarget.clientWidth))}
          >
          ...
          {displayImages.length > 1 && (
            <>
              ...
              <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-1.5 z-20 pointer-events-none">
                 {displayImages.map((_:any, i:number) => (<div key={i} className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${i === currentImgIndex ? 'bg-white scale-125' : 'bg-white/50'}`} />))}
              </div>
            </>
          )}
        </div>
        ...
        <div className="p-5 bg-white rounded-t-[24px] -mt-6 relative z-10 min-h-[50vh]">
          ...
          <div onClick={() => room.author_id && router.push(`/profile/${room.author_id}`)} className="flex items-center justify-between mb-6 pb-6 border-b border-gray-100 cursor-pointer hover:bg-gray-50 p-2 -ml-2 rounded-xl transition-colors group">
            ...
            <div className="mb-8">
              ...
              <div className="border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                <div className="p-4 bg-white flex items-center justify-between gap-4">
                  <div className="flex-1 min-w-0"><div className="text-[15px] font-bold text-gray-900 truncate">{room.address_name || `${room.city_name} (未提供具体地址)`}</div></div>
                  <button onClick={() => { window.open(room.latitude && room.longitude ? `https://www.openstreetmap.org/?mlat=${room.latitude}&mlon=${room.longitude}#map=16/${room.latitude}/${room.longitude}` : `https://www.openstreetmap.org/search?query=${encodeURIComponent(room.address_name || room.city_name)}`, '_blank'); }} className="w-10 h-10 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 hover:bg-blue-100 transition-colors"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg></button>
                </div>
            ...
            <>
              {canApply && (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 mt-2">
                    <button onClick={() => handleChatWith(room.author_id)} className="py-3.5 px-4 rounded-xl font-bold text-[14px] bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors flex items-center justify-center gap-1.5">
                      <svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" /></svg>
                      私聊
                    </button>
              ...
              {['pending', 'approved', 'paid'].includes(myBooking?.status) && (
                <div className="mt-4 pt-4 border-t border-gray-100 flex gap-3">
                  ...
                  <button onClick={() => router.push('/my-bookings')} className="flex-1 py-3.5 rounded-xl font-bold text-[14px] bg-gray-50 text-gray-700 hover:bg-gray-100 transition-colors flex items-center justify-center gap-2">管理订单</button>
                </div>
              )}
            </>
          )}
