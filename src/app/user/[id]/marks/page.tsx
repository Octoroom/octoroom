// src/app/user/[id]/marks/page.tsx
'use client';
...
export default function UserMarksPage() {
  ...
  useEffect(() => {
    const fetchMarks = async () => {
      ...
      try {
        // 1. 去 bookmarks 表查出该博主收藏过的所有 帖子ID
        const { data: marksData } = await supabase
          .from('bookmarks') // 查询的是收藏表
        ...
        const postIds = marksData.map(item => item.post_id);
        ...
        const enrichedPosts = marksData.map(mark => {
          const p = postMap.get(mark.post_id);
          if (!p) return null; // 容错：跳过已被删除的帖子
  ...
  return (
    <div className="max-w-2xl mx-auto min-h-screen bg-gray-50 border-x border-gray-100 pb-20">
      <div className="sticky top-0 bg-white/80 backdrop-blur-md z-40 flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4 border-b border-gray-100">
        <h1 className="text-[18px] sm:text-xl font-black text-gray-900">Ta 收藏的动态</h1>
      ...
      <div className="p-4 sm:p-6">
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-gray-100 shadow-sm">
            <p className="text-[14px] text-gray-400 font-medium">Ta 还没有收藏任何动态哦</p>
          </div>
        ) : (
          <PostList 
            posts={posts} 
