// src/app/profile/marks/page.tsx
'use client';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (marks && marks.length > 0) {
        ...
        if (postsData) {
          // 3. 🌟 关键：提取所有作者的 ID，单独去获取资料，防止"未知用户"
          ...
          const enrichedPosts = postsData.map(p => ({
            ...
            _interactions: {
              ...
              markedByMe: true, // 因为这是收藏列表，所以星标必定是亮起的
              repostedByMe: myReposts.includes(p.id),
              isFollowingAuthor: false
            }
          }));
          
          // 按照收藏的时间顺序重新排序（保持原本的瀑布流顺序）
    ...
    <div className="min-h-screen bg-octo-cream p-4">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center justify-between mb-6 sticky top-0 bg-octo-cream/90 backdrop-blur-md z-10 py-2 -mx-4 px-4 sm:-mx-6 sm:px-6">
          <h1 className="text-2xl font-black text-orange-600">我的收藏 (Marks)</h1>
          <Link href="/profile" className="text-sm font-bold text-gray-500 hover:text-orange-600 transition-colors">
            ← 返回个人主页
          </Link>
        </div>
