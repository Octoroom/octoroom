...
// 房间卡片组件 (用于列表展示)
function RoomCardSlider({ room, images, isMarked, toggleMark }: { room: any, images: string[], isMarked: boolean, toggleMark: (e:any)=>void }) {
...
export default function MyMarksPage() {
  ...
  const [activeTab, setActiveTab] = useState<'rooms' | 'posts' | 'companions'>('posts'); 
  // 🌟 布局模式状态：默认瀑布流 waterfall 
  ...
  const fetchMarks = async () => {
    ...
    try {
      ...
      if (activeTab === 'rooms') {
        ...
        const { data: compMarks, error: compMarksError } = await supabase
          .from('companion_marks')
          .select('room_id')
          .eq('user_id', user.id);
  ...
  const handleToggleMark = async (e: React.MouseEvent, itemId: string, type: 'rooms' | 'companions' | 'posts') => {
  ...
  return (
    <main className="flex-1 max-w-[640px] w-full min-h-screen border-r border-gray-100 bg-gray-50 flex flex-col relative">
      <div className="flex items-center justify-between px-4 sm:px-6 py-4 bg-white border-b border-gray-100 sticky top-0 z-40">
        ...
        {/* 🌟 核心：瀑布流与列表展示切换器 (瀑布流在前) */}
        <div className="flex items-center gap-1.5 bg-gray-50 p-1 rounded-lg border border-gray-100">
          <button 
            onClick={() => setViewMode('waterfall')} 
            className={`p-1.5 rounded-md transition-all ${viewMode === 'waterfall' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}
            title="瀑布流排版"
          >
      ...
      <div className="flex w-full bg-white border-b border-gray-100 sticky top-[69px] z-30">
        <button onClick={() => setActiveTab('posts')} className={`flex-1 py-3 text-[14px] relative transition-colors ${activeTab === 'posts' ? 'font-bold text-gray-900' : 'font-medium text-gray-500'}`}>日常动态</button>
        <button onClick={() => setActiveTab('companions')} className={`flex-1 py-3 text-[14px] relative transition-colors ${activeTab === 'companions' ? 'font-bold text-gray-900' : 'font-medium text-gray-500'}`}>找搭子</button>
        <button onClick={() => setActiveTab('rooms')} className={`flex-1 py-3 text-[14px] relative transition-colors ${activeTab === 'rooms' ? 'font-bold text-gray-900' : 'font-medium text-gray-500'}`}>精选房子</button>
        <div className="absolute bottom-0 h-[3px] bg-gray-900 rounded-full transition-all duration-300 w-12" style={{ left: activeTab === 'posts' ? '16.6%' : activeTab === 'companions' ? '50%' : '83.3%', transform: 'translateX(-50%)' }}></div>
      </div>
        ...
        {loading ? (
          ...
          markedRooms.length === 0 ? (
            <div className="text-center py-32">
              <h3 className="text-lg font-bold text-gray-900">房源收藏夹空空如也</h3>
            ...
            <div className="flex flex-col gap-5">
              {markedRooms.map((room) => (
                <div key={room.id} onClick={() => router.push(`/rooms/${room.id}`)} className="bg-white overflow-hidden shadow-sm hover:shadow-md cursor-pointer border border-gray-100 group transition-all rounded-[20px] flex flex-col">
                  <RoomCardSlider images={getDisplayImages(room)} room={room} isMarked={true} toggleMark={(e)=>handleToggleMark(e, room.id, 'rooms')} />
            ...
            <div className={viewMode === 'waterfall' ? "columns-2 gap-3 space-y-3" : "flex flex-col gap-4"}>
              {markedPosts.map((post) => (
                 <div 
                   ...
                   className={`bg-white rounded-[16px] overflow-hidden shadow-sm hover:shadow-md transition-all cursor-pointer border border-gray-100 group relative ${viewMode === 'waterfall' ? 'break-inside-avoid' : 'flex flex-col'}`}
                 >
                   {(() => {
                     ...
                     return imgs && imgs.length > 0 ? (
                       <div className="relative w-full bg-gray-100 overflow-hidden">
                         <img src={imgs[0]} alt="post" className={`w-full object-cover group-hover:scale-105 transition-transform duration-500 ${viewMode === 'list' ? 'max-h-80' : ''}`} />
                       </div>
                     ) : (
                   ...
                   <div className="p-3">
                     <p className={`font-medium text-gray-800 leading-snug break-words mb-2 ${viewMode === 'list' ? 'text-[15px] line-clamp-3' : 'text-[13px] line-clamp-2'}`}>
                       {post.content}
                     </p>
                     ...
                     <div className="flex items-center justify-between mt-3 pt-2 border-t border-gray-50">
                       <div className="flex items-center gap-1.5 flex-1 min-w-0">
                         <img 
                           ...
                           alt="avatar" 
                           className="w-5 h-5 rounded-full bg-gray-100 object-cover flex-shrink-0" 
                         />
